...
public class ResultingPanel extends javax.swing.JPanel implements ValidationStateChangedListener{
    ...
    public ResultingPanel(WizardController wizardController, Map wizardData,String resultMode) {
        ...
        initComponents();
        mode = resultMode;
        FlurstueckSchluessel splitCandidate = (FlurstueckSchluessel) wizardData.get(SplitActionChoosePanel.KEY_SPLIT_CANDIDATE);
        if(splitCandidate == null){
            try{
                ArrayList<FlurstueckSchluessel> joinKeys= (ArrayList<FlurstueckSchluessel>)wizardData.get(JoinActionChoosePanel.KEY_JOIN_KEYS);
        ...
        if(mode.equals(SPLIT_ACTION_MODE) || mode.equals(SPLIT_JOIN_ACTION_MODE)){
            ...
            int splitCount = (Integer) wizardData.get(SplitActionChoosePanel.KEY_SPLIT_COUNT);
            log.debug("Anzahl neu erstellter Flurstücke: "+splitCount);
            
            
            for(int i=0;i<splitCount;i++){
                FlurstueckChooser tmpFlst = new FlurstueckChooser(FlurstueckChooser.CREATION_MODE);
                ...
                if(splitCandidate != null){
                    ...
                    tmpFlst.doAutomaticRequest(FlurstueckChooser.AutomaticFlurstueckRetriever.COPY_CONTENT_MODE,splitCandidate);
                }else {
    ...
    public synchronized void refreshCount(){
        if(mode.equals(SPLIT_ACTION_MODE) || mode.equals(SPLIT_JOIN_ACTION_MODE)){
            ...
            Integer splitCount = (Integer) wizardData.get(SplitActionChoosePanel.KEY_SPLIT_COUNT);
    ...
    public void validationStateChanged(Object validatedObject) {
        ...
        splitKeys.clear();
        Iterator<FlurstueckChooser> splitMembers = resultCandidates.iterator();
        while(splitMembers.hasNext()){
            FlurstueckChooser curSplitMember = splitMembers.next();
            ...
            if(flurstuecke != null && flurstuecke.size() >1){
                ...
                for(FlurstueckChooser flurstueckToTest:flurstuecke){
                    ...
                    for(FlurstueckChooser curFlurstueck:flurstuecke){
                        FlurstueckSchluessel schluesselToTest = flurstueckToTest.getCurrentFlurstueckSchluessel();
                        FlurstueckSchluessel curSchluessel = curFlurstueck.getCurrentFlurstueckSchluessel();
                        if(schluesselToTest != null && curSchluessel != null){
                            log.debug("Zu testende Schluessel sind != null");
                            if(FlurstueckSchluessel.FLURSTUECK_EQUALATOR.pedanticEquals(schluesselToTest,curSchluessel)){
                                counter++;
                                if(counter > 1){
    ...
    private void initComponents() {
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(spAction, javax.swing.GroupLayout.PREFERRED_SIZE, 353, javax.swing.GroupLayout.PREFERRED_SIZE))
        );
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addComponent(lblErgebnis)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(spAction, javax.swing.GroupLayout.PREFERRED_SIZE, 136, javax.swing.GroupLayout.PREFERRED_SIZE)
